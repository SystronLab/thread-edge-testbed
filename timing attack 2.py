...
class ot_device:
    ...
    def run_command(self, command):
        ...
        if "scan energy" in command:
            time.sleep(17 * SCAN_LENGTH * 0.001)  # wait for the 16 channels to be scanned and then wait a further scan length
        if DEBUG:
            print("\n" + command)
    ...
    def ping(self, address):
        res = self.run_command("ping " + str(address))
        res_arr = res.split(" ")
        try:
            drop_rate = float(
...
def timing_attack_demo():
    ...
    for device in thread_devices:
        for receiver in thread_devices:
            if device != receiver:
                ...
                # Store the result: source port, destination port, elapsed time, and drop rate
    ...
    for result in timing_results:
        if len(result) == 4:
            print(f"From {result[0]} to {result[1]}: Time: {result[2]:.6f} seconds, Drop Rate: {result[3]}")
        else:
...
def analyze_timing(results):
    times = [res[2] for res in results if res[3] != 'err']
    if times:
        average_time = sum(times) / len(times)
        print(f"\nAverage Response Time: {average_time:.6f} seconds")

        anomalies = [res for res in results if res[3] != 'err' and abs(res[2] - average_time) > 0.01]  # Example threshold
        if anomalies:
            print("\nPotential Timing Anomalies Detected:")
            for anomaly in anomalies:
                print(f"From {anomaly[0]} to {anomaly[1]}: Time: {anomaly[2]:.6f} seconds, Drop Rate: {anomaly[3]}")
        else:
            print("No significant timing anomalies detected.")
    else:
        print("No valid timing results found for analysis.")
